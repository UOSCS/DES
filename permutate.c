...
{
    bits_64 result = 0, bit_mask = 0xFC0000000000;
    for(int i = 0, row, col; i < NUM_OF_S_BOX; i++)
    {
        bits_64 take_6_bits = right_XOR_key & bit_mask;
        take_6_bits >>= ((NUM_OF_S_BOX - i - 1) * 6);
        row = (take_6_bits & 1) + ((take_6_bits & 0x20) >> 4);
        col = (take_6_bits >> 1) & 0xF;
        result |= S_BOX[i][row * 16 + col] << ((NUM_OF_S_BOX - i - 1) * 4);
        bit_mask >>= 6;
    }
    return result;
}
